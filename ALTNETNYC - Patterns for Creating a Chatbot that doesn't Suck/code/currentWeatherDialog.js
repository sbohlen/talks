...
function buildPrettyDateString(input) {
    return (input.getMonth() + 1) + "/" + input.getDate() + "/" + input.getFullYear();
}

function buildPrettyPrecipChanceString(percent) {
    return (percent * 100) + "%";
}

function populateWeatherReportMessageTemplate(template, forecastLocation, weather, hasPrecip) {
    var message = template.replace("${date}", buildPrettyDateString(weather.date))
...
function getForecastDateRange(session, args) {
    ...
    if (date) {
        var extractedDate = new Date(date.resolution.values[0].value);
